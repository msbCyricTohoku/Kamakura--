...
bool CodeEditor::find(QString query, bool caseSensitive, bool wholeWords) {
  ...
  QTextDocument::FindFlags searchOptions =
      getSearchOptionsFromFlags(caseSensitive, wholeWords);
...
bool CodeEditor::findBackward(QString query, bool caseSensitive,
                              bool wholeWords) {
  int cursorPositionBeforeCurrentSearch = textCursor().position();
...
void CodeEditor::replace(QString what, QString with, bool caseSensitive,
                         bool wholeWords) {
...
void CodeEditor::replaceAll(QString what, QString with, bool caseSensitive,
                            bool wholeWords) {
  ...
  if (replacements == 0) {
    ...
    emit(findResultReady("Input file searched. Replaced " +
                         QString::number(replacements) + " keywords."));
  }
...
CodeEditor::getSearchOptionsFromFlags(bool caseSensitive, bool wholeWords) {
...
void CodeEditor::handleTextChanged() {
  ...
  metrics.wordCount =
      text.split(QRegularExpression("\\b\\w+\\b"), Qt::SkipEmptyParts).size();
...
void CodeEditor::setWordWrap(bool enable) {
  ...
  setLineWrapMode(enable ? QPlainTextEdit::WidgetWidth
...
void CodeEditor::indentSelection(bool unindent) {
  ...
  int endPos = afterEnd.isValid() ? afterEnd.position() - 1
...
void CodeEditor::handleAskGemini() {
  ...
  if (aiRequester->geminiKey().isEmpty()) {
    ...
    QString key = QInputDialog::getText(this, tr("Gemini API Key"),
                                        tr("Enter your Google Gemini API key"),
                                        QLineEdit::Normal, QString(), &ok);
    if (!ok || key.isEmpty())
      return;
...
void CodeEditor::toggleComment() {
  ...
  for (QTextBlock block = startBlock; block.isValid(); block = block.next()) {
    ...
    bool commented =
        text.mid(firstNonSpace, commentPrefix.length()) == commentPrefix;
    lineCursor.setPosition(block.position() + firstNonSpace);
    if (commented) {
      lineCursor.movePosition(QTextCursor::NextCharacter,
                              QTextCursor::KeepAnchor, commentPrefix.length());
      lineCursor.removeSelectedText();
    } else {
